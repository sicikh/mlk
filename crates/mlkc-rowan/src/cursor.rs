...
use crate::{
    ...
    cursor::node::Siblings,
    green::{self, GreenElement, GreenElementRef, GreenNodeData, GreenTokenData, RawSyntaxKind},
};
...
impl WeakGreenElement {
    fn new(green: GreenElementRef) -> Self {
        match green {
            NodeOrToken::Node(ptr) => {
                Self::Node {
                    ptr: ptr::NonNull::from(ptr),
                }
            },
            NodeOrToken::Token(ptr) => {
                Self::Token {
                    ptr: ptr::NonNull::from(ptr),
                }
            },
