...
use crate::{
    ...
    WalkEvent,
    cursor::{NodeData, SyntaxElement, SyntaxToken, SyntaxTrivia},
    green::{Child, Children, GreenElementRef, Slot},
};
...
impl SyntaxNode {
    ...
    pub fn siblings(&self, direction: Direction) -> impl Iterator<Item = Self> + use<> {
        iter::successors(Some(self.clone()), move |node| {
            match direction {
    ...
    pub fn siblings_with_tokens(
        ...
        iter::successors(Some(me), move |el| {
            match direction {
    ...
    pub fn descendants(&self) -> impl Iterator<Item = Self> + use<> {
        self.preorder().filter_map(|event| {
            match event {
    ...
    pub fn descendants_with_tokens(
        ...
        self.preorder_with_tokens(direction).filter_map(|event| {
            match event {
                WalkEvent::Enter(it) => Some(it),
                WalkEvent::Leave(_) => None,
            }
        ...
        loop {
            ...
            res = match &res {
                ...
                NodeOrToken::Node(node) => {
                    match node.child_or_token_at_range(range) {
                        Some(it) => it,
                        None => return res,
                    }
                },
...
impl Preorder {
    ...
    #[cold]
    fn do_skip(&mut self) {
        self.next = self.next.take().map(|next| {
            match next {
        ...
        self.next = self.next.take().map(|next| {
            match next {
                WalkEvent::Enter(first_child) => {
                    WalkEvent::Leave(first_child.parent().unwrap().into())
                },
                WalkEvent::Leave(parent) => WalkEvent::Leave(parent),
            }
        })
...
impl SyntaxSlots {
    ...
    fn map_slot(&self, slot: &Slot, slot_index: u32) -> SyntaxSlot {
        match slot {
            ...
            Slot::Token { rel_offset, token } => {
                SyntaxSlot::Token(SyntaxToken::new(
                    token,
                    self.parent.clone(),
                    slot_index,
            ...
            Slot::Node { rel_offset, node } => {
                SyntaxSlot::Node(SyntaxNode::new_child(
                    node,
                    self.parent.clone(),
                    slot_index,
                    self.parent.offset() + rel_offset,
