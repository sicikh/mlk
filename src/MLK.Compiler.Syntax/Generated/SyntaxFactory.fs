...
module SyntaxFactory =
    let makeSyntax (kind : RawSyntaxKind) (children : ParsedChildren) : GreenNode =
        ...
        | SyntaxKind.AndPat ->
            ...
            let slots, elements =
                match elements with
                | element :: tail when Pat.CanCast element.RawKind -> RawNodeSlots.addPresent slots, tail
        ...
        | SyntaxKind.QTy ->
            ...
            match elements with
            | [] -> RawNodeSlots.intoNode (SyntaxKind.toRaw kind) children slots
            | _ -> GreenNode.Create (kind.Err.Raw, ParsedChildren.elements children |> Seq.map Some)
