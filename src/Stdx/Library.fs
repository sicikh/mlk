...
type ControlFlow<'a, 'b> =
    ...
    member x.isContinue with get() =
        match x with
        | Continue _ -> true
        | Break _ -> false

    member x.isBreak with get() =
        match x with
        | Continue _ -> false
        | Break _ -> true

    member x.ContinueValue with get() =
        match x with
        | Continue v -> v
        | Break _ -> failwith "not a continue value"

    member x.BreakValue with get() =
        match x with
        | Break v -> v
...
module Seq =
    ...
    let tryIter action source =
        tryFold (fun _ -> action) () source

    let tryFoldResult folder state source =
        tryFold
            (fun s i -> folder s i |> ControlFlow.fromResult)
            state
            source
    ...
    let tryFoldOption folder state source =
        tryFold
            (fun s i -> folder s i |> ControlFlow.fromOption)
            state
            source
    ...
    let tryFold2Option folder state source1 source2 =
        tryFold2
            (fun s i1 i2 -> folder s i1 i2 |> ControlFlow.fromOption)
            state
            source1
            source2
        |> ControlFlow.toOption
